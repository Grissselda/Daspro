...
def kumpulBase(jinName):
    global NthKumpul
    NthKumpul += 1

    randomArr = LCG(1, NthKumpul)
    Npasir = randomArr[0]
    Nbatu = randomArr[1]
    Nair = randomArr[2]

    findBahanBangunan("pasir","padatan",Npasir)
    findBahanBangunan("batu", "padatan", Nbatu)
    findBahanBangunan("air", "cairan", Nair)

    findJin(jinName,Npasir,Nbatu,Nair)


    return [Npasir,Nbatu,Nair]

def kumpul(jinName) :
    arr = kumpulBase(jinName)
    pasir = arr[0]
    batu = arr[1]
    air = arr[2]
    print(f"Jin menemukan {pasir} pasir , {batu} batu , dan {air} air.")

def batchkumpul(jinGroup):
    if length(jinGroup) <=0 :
        print("Kumpul gagal. Anda tidak punya jin pengumpul. Silahkan summon terlebih dahulu.")
    else :
        pasir = 0
        batu = 0
        air = 0
        print(f"Mengerahkan {length(jinGroup)} jin untuk mengumpulkan bahan")

        for i in range(length(jinGroup)) :
            bahan = kumpulBase(jinGroup[i])
            pasir += int(bahan[0])
            batu += int(bahan[1])
            air += int(bahan[2])
...
def ayamberkokok():
    ...
    exit(True)
    # False atau True di parameter exit ?
    return 0
